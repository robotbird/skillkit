#!/usr/bin/env node
// 诊断脚本:能否把本地文件上传到 Vercel。分两段独立测试:
//
//   A) 直接用 @vercel/blob 的 put() 上传
//      —— 验证「Blob 存储 + BLOB_READ_WRITE_TOKEN」本身通不通(完全绕开 HTTP / Hono / 路由)
//      —— 需要 BLOB_READ_WRITE_TOKEN 环境变量;没有就跳过 A。
//
//   B) 通过 HTTP POST {SHARE_BASE_URL}/api/share 上传
//      —— 验证线上分享接口端到端(路由 + 函数 + 存储)
//      —— 这是桌面客户端 share:create 实际走的路径。
//
// 用法:
//   node scripts/test-vercel-upload.mjs [本地文件路径]
//   BLOB_READ_WRITE_TOKEN=xxx node scripts/test-vercel-upload.mjs        # 跑 A 段需要
//   SHARE_BASE_URL=https://skillkit.net node scripts/test-vercel-upload.mjs   # B 段目标(默认 skillkit.net)
//
// 结论速读:
//   A 成功 + B 404       → Blob 存储没问题,卡在 HTTP 路由层(当前已知问题)
...
// ============ B) HTTP POST /api/share ============
console.log('━━━ B) HTTP POST {SHARE_BASE_URL}/api/share ━━━');
...
const url = `${SHARE_BASE_URL}/api/share`;
...
try {
  ...
  } else if (res.status === 404 && /NOT_FOUND/i.test(text)) {
    console.log(`  ✗ 这是 Vercel 路由层 404(请求根本没进函数)—— 即当前的多段路由问题`);
  } else {
    console.log(`  ✗ 非 2xx`);
  }
} catch (e) {
