...
Vercel deploy: Root Directory = **repo root** (so the `shared/` sibling gets bundled; rooting at `server/` drops it and breaks the `../../shared/types.js` import). `vercel.json` sets `framework:null`, an `installCommand` with `--omit=dev --ignore-scripts` to skip the Electron binary download and better-sqlite3 native compile (neither is used by the function), and `buildCommand: "echo skip-app-build"` — **must be a real no-op command, not `null`** (null falls back to the default `npm run build`, which fails with `tsc not found` since devDeps are omitted). `.vercelignore` excludes desktop-only dirs. Env: `SHARE_STORE=blob`, `BLOB_READ_WRITE_TOKEN`, `CRON_SECRET`. Client `SHARE_BASE_URL` defaults to `https://skillkit.net` (override locally with `SKILLKIT_SHARE_BASE_URL`).
