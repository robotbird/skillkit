...
- `api/[[...route]].ts` (repo root) — Vercel entry: `export default handle(app)` where `handle` is from **`@hono/node-server/vercel`** (NOT `hono/vercel`), importing `app` from `./lib/app.js`. `@hono/node-server/vercel`'s `handle` returns the Node-style `(req, res) => void` that writes the response through `res` — the correct shape for `runtime: 'nodejs'` + a `default` export. Two wrong adapters, both observed failing in production: (1) `hono/vercel`'s `handle` returns a Web-style `Response`, which `@vercel/node` ignores on a `default` export (build warns "default export returned a `Response`", client gets 404/empty); (2) a named `export const fetch = (req) => app.fetch(req)` (Web API) makes the `[[...route]]` catch-all only match single-segment paths (`/api/share` works, `/api/share/a/b` returns Vercel `NOT_FOUND`). The `[[...route]]` optional catch-all routes **all** `/api/*` to one function — **do not add a catch-all rewrite for `/api/*`** (it would collapse nested paths and break `c.req.param('id')`).
