...
- `api/[...route].ts` (repo root) — Vercel entry: `export default handle(app)` where `handle` is from **`@hono/node-server/vercel`** (NOT `hono/vercel`), importing `app` from `./lib/app.js`. `@hono/node-server/vercel`'s `handle` returns the Node-style `(req, res) => void` that writes the response through `res` — the correct shape for `runtime: 'nodejs'` + a `default` export. (`hono/vercel`'s `handle` instead returns a Web-style `Response`, which `@vercel/node` ignores on a `default` export — build warns "default export returned a `Response`", client gets 404/empty.) **Use `[...route]` (required catch-all, SINGLE bracket), NOT `[[...route]]`**: `[[...]]` is a Next.js optional-catch-all convention that `@vercel/node` mis-routes as a single dynamic segment — only single-segment paths (`/api/share`) reached the function; multi-segment (`/api/share/a/b`, `/api/share/a/b/meta`) returned Vercel `NOT_FOUND` without ever invoking it. `[...route]` matches 1+ segments and covers every real route (bare `/api`/`/` health is intentionally not served; cron `/api/sweep` works). **Do not add a catch-all rewrite for `/api/*`** (it would collapse nested paths and break `c.req.param('id')`).
