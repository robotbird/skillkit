...
- Vercel entries — **one function file per route depth** (NOT a catch-all): `api/sweep.ts`, `api/share/index.ts` (POST `/api/share`), `api/share/[id].ts` (GET `/api/share/:id` HTML), `api/share/[id]/meta.ts`, `api/share/[id]/zip.ts`. Each is identical: `export const config = { runtime: 'nodejs', maxDuration: 60 }; export default handle(app)` where `handle` is from **`@hono/node-server/vercel`** (NOT `hono/vercel`), importing `app` from `../lib/app.js` (depth-adjusted). The file's *location* determines which URL depths reach a function; Hono does the actual routing by `req.url` (with `basePath('/api')`). **Why not a catch-all `[[...route]]`/`[...route]`**: on this project `@vercel/node` mis-routes a catch-all as a single dynamic segment — only single-segment `/api/<x>` reached the function; every multi-segment path (`/api/share/<id>`, `/api/share/<id>/meta`) returned Vercel `NOT_FOUND` without invoking it. One-file-per-route avoids the catch-all entirely and is the most basic Vercel routing. `@hono/node-server/vercel`'s `handle` returns the Node-style `(req, res) => void` that writes through `res` — the correct shape for `runtime: 'nodejs'` + a `default` export. (`hono/vercel`'s `handle` returns a Web-style `Response`, which `@vercel/node` ignores on a `default` export — build warns "default export returned a `Response`", client gets 404/empty; and a named `export const fetch` made the catch-all behave even worse. Don't use either.) **Do not add a catch-all rewrite for `/api/*`** (it would collapse nested paths and break `c.req.param('id')`).
