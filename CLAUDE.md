...
- `api/[[...route]].ts` (repo root) — Vercel entry: `export default handle(app)` via `hono/vercel`, importing `app` from `./lib/app.js`. The `[[...route]]` optional catch-all routes **all** `/api/*` to one function — **do not add a catch-all rewrite for `/api/*`** (it would collapse nested paths and break `c.req.param('id')`).
