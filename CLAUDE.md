...
- `api/lib/app.ts` — side-effect-free Hono `app` (`.basePath('/api')`), all routes, storage via `getStore()`. Lives inside `/api` so `@vercel/node` compiles it (TS outside `/api` is never compiled). Importing it must NOT trigger `serve()`/`setInterval`/CLI/`ensureDir`. Sibling `store.ts`, `store-blob.ts`, `types.ts` share the same constraint.
...
- `api/[[...route]].ts` (repo root) — Vercel entry: `export default handle(app)` via `hono/vercel`, importing `app` from `./lib/app.js`. The `[[...route]]` optional catch-all routes **all** `/api/*` to one function — **do not add a catch-all rewrite** (it would collapse nested paths and break `c.req.param('id')`).
...
Vercel deploy: Root Directory = **repo root**. `vercel.json` sets `framework:null` (no static frontend), `installCommand` with `--omit=dev --ignore-scripts` to skip the Electron binary download and better-sqlite3 native compile (neither is used by the function), `outputDirectory: "."` (without it Vercel errors "No Output Directory named dist"), and the `api/[[...route]].ts` function with `maxDuration: 60`. **No `buildCommand` and no `includeFiles`** — neither is needed, and both were the source of repeated failed deploys. The hard constraint: **`@vercel/node` only compiles TS *inside `/api`*** (and a `server` entrypoint). It does NOT run the project `buildCommand` during the function build (that's for frontend frameworks; `framework:null` skips it entirely), and `includeFiles` copies files verbatim without compiling `.ts`→`.js`. So any TS the function imports *must* live physically inside `/api` — which is why the app/store/types moved into `api/lib/`. `@vercel/node` then compiles all of `/api` (including `lib/**`) and nft traces the import graph automatically. `api/tsconfig.json` (NodeNext) lets `@vercel/node`'s typecheck resolve the `.js` import specs to their `.ts` sources (a `.js` import only rewrites to `.ts` under NodeNext/Bundler, not default `moduleResolution: node`); the root `tsconfig.json` is excluded by `.vercelignore` so the nearest-tsconfig walk-up hits `api/tsconfig.json`. `.vercelignore` excludes desktop-only dirs (`electron/`, `src/`, `build/`, etc.) — **root-file patterns must be anchored with a leading `/`** (e.g. `/tsconfig.json`), since unanchored `tsconfig.json` matches at any depth and would also drop `server/tsconfig.json`. Env: `SHARE_STORE=blob`, `BLOB_READ_WRITE_TOKEN`, `CRON_SECRET`. Client `SHARE_BASE_URL` defaults to `https://skillkit.net` (override locally with `SKILLKIT_SHARE_BASE_URL`).
